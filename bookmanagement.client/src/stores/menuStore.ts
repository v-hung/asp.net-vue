...
import type { MenuItem } from "@/generate-api";
...
export const useMenuStore = defineStore("menu", () => {
  ...
  const getAllMenus = async () => {
    try {
      const response = await menuApi.apiMenuGet();

      menus.value = response.data.map((menu) => {
        return {
          ...menu,
          icon: menu.url ? MENU_PATH_ICONS[menu.url].icon ?? null : null,
        };
      });
    } catch (error) {
      console.log(error);
  ...
  const hasAccessPath = (path: string) => {
    ...
    return menus.value.some(
      (menu) =>
        menu.url === path || menu.children.some((child) => child.url === path)
    );
  };
    ...
    for (const menu of menus.value) {
      if (menu.url === path) {
        ...
        for (const child of menu.children) {
          if (child.url === path) {
            currentMenu.push(menu);
            currentMenu.push({ ...child, icon: null });
            break;
          }
        }
      }
